...
def main():
    ...
    for rucksack in rucksacks:
        ...
        sum_priorities += sum(
            priorites_mapping[common_letter] for common_letter in compart1 & compart2
        )
    ...
    for i in range(0, len(rucksacks), batch):
        sum_priorities += sum(
            priorites_mapping[common_letter]
            for common_letter in set.intersection(*map(set, rucksacks[i : i + batch]))
        )
