def main():
    ...
    #other method 
    index = next(window+i for i in range(0, len(signal)-window) if len(set(signal[i:i+window]))==window)
    ...
    #other method
    index2 = next(window+i for i in range(index, len(signal)-window) if len(set(signal[i:i+window]))==window)
