...
def total_size_of_low_memory_dir(directory:Directory, low_memory_threshold:int):
    if directory.size <= low_memory_threshold:
        return directory.size + sum(total_size_of_low_memory_dir(dire, LOW_MEMORY_THRESHOLD) for dire in directory.children if isinstance(dire, Directory))
    return sum(total_size_of_low_memory_dir(dire,LOW_MEMORY_THRESHOLD) for dire in directory.children if isinstance(dire, Directory))


def min_dir_size_above(directory: Directory, min_size:int):
    return min([directory.size ] + [min_dir_size_above(dire, min_size) for dire in directory.children if isinstance(dire, Directory) and dire.size >=min_size])
        
def min_dir_size_above_safe(directory: Directory, min_size:int):
    if directory.size < min_size:
        raise ValueError(f"The size of the directory ({directory.size}) is below {min_size=}")
        ...
        if prompt.startswith("$"):
            ...
            if command[0] == "cd":
                if command[1] == "..":
                    ...
                    current_directory = next(dire for dire in current_directory.children if dire.name == command[1])
        else:
            ...
            if name not in (dire.name for dire in current_directory.children):
                if dir_or_size.isnumeric():
                    ...
                    current_directory.children.append(File(name=name, size=size, parent=current_directory))
                else:
                    current_directory.children.append(Directory(name=name, parent=current_directory))
